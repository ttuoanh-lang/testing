...
**Persistence** ([src/lib/db.ts](src/lib/db.ts)): `better-sqlite3` against a local `game.db` file (gitignored), auto-creating the `scores` table on import. This is workshop-local storage only — it does not survive Vercel's serverless filesystem between requests/deploys. A later migration swaps these calls for `@supabase/supabase-js` behind the same API routes (see STACK.md's migration path); don't set up Supabase or an ORM ahead of that step.
